...
- **HALLAZGO IMPORTANTE: el pipeline de codegen (`build_runner`) nunca funciono en este proyecto**, ni siquiera para `MacroGoal` (que ya existia antes de hoy). Causa raiz doble: (1) `macro_goal.dart` estaba escrito con sintaxis de freezed 3.x (`abstract class X with _$X`) pero `pubspec.yaml` fijaba `freezed: ^2.5.7` (sintaxis vieja) - se corrigio subiendo `freezed`/`freezed_annotation` a `^3.0.0`; (2) con eso corregido, `build_runner` seguia crasheando con `Exception: Missing implementation of visitDotShorthandInvocation` porque el `analyzer` que trae `build_runner 2.5.4` (tope transitivo, no sube solo con `pub upgrade`) no sabe parsear sintaxis "dot-shorthand" que ya usan los sources de Flutter 3.44/Dart 3.12. Se probo forzar `analyzer` mas nuevo via `dependency_overrides`, pero rompe `dart_style` (su API de AST asume la version vieja) - es una cadena de incompatibilidades transitivas, no un fix de una linea. La unica salida limpia es un bump mayor coordinado de `build_runner`/`riverpod`/`riverpod_generator`/`go_router` a la vez (Riverpod 2.x->3.x y go_router 14.x->17.x son breaking changes reales), que es una decision de stack (CLAUDE.md seccion 4) - **pendiente de decidir con Oscar, no hacer unilateral**.
- **Mitigacion aplicada mientras tanto:** `MacroGoal`, `DayMealEntry`, `DayMacroTotals` (`lib/models/*.dart`) estan escritos a mano (clases inmutables simples con `fromJson`/`toJson` manual donde hace falta), NO con `@freezed`. `flutter analyze` corre limpio (2 warnings menores preexistentes, 0 errores) sin necesitar `build_runner`. Cuando se resuelva el bump mayor de arriba, se puede volver a usar freezed en estos 3 modelos.
- `core/api` (api_client, nutriflow_api, providers) y `core/supabase/supabase_bootstrap.dart` siguen igual. Auth gate (`_AuthGate` en `app.dart` + `LoginScreen`) sigue como se documento el 2026-07-16.
- Las demas carpetas de `features/` (`logging`, `meal_plan`, `onboarding`, `fasting`, `body_metrics`, `favorites`) siguen VACIAS. `go_router` sigue instalado sin wirear. Sigue sin tests (`test/` vacio o inexistente).

**Aprendizaje para no repetir:** si `dart run build_runner build` crashea con una excepcion de `analyzer` (`visitDotShorthandInvocation` o similar "Missing implementation of visitX"), NO asumir que es un bug en el codigo nuevo - primero correr `flutter analyze` solo para descartar errores reales de codigo. Si `flutter analyze` sale limpio pero `build_runner` crashea, es un desfase de version entre el `analyzer` que usa el SDK de Flutter (siempre al dia) y el `analyzer` transitivo de `build_runner` (se queda atras). Revisar `flutter pub outdated` para ver que tan atras estan `build_runner`/`freezed`/`riverpod`/`go_router` de sus majors antes de gastar tiempo en `dependency_overrides` puntuales - probablemente hace falta el bump coordinado, no un parche.

**Siguiente accion concreta (decidir con Oscar):** (a) decidir si/cuando hacer el bump mayor coordinado (`build_runner`, `freezed` ya en 3.x, `riverpod`+`riverpod_generator` a 3.x/4.x, `go_router` a 17.x) para recuperar codegen real, o seguir con modelos a mano mientras el arbol de modelos siga chico; (b) wirear `go_router` ya que esta instalado sin usar; (c) pantalla de logging manual de comidas (hoy no hay forma de escribir a `meal_logs` desde la app, solo de leerlos).
