...
- Las demas carpetas de `features/` (`logging`, `meal_plan`, `onboarding`, `fasting`, `body_metrics`, `favorites`) siguen VACIAS. `go_router` sigue instalado sin wirear (ahora en 17.x). Sigue sin tests (`test/` vacio o inexistente).
...
**Siguiente accion concreta (decidir con Oscar):** (a) wirear `go_router` ya que esta instalado sin usar; (b) pantalla de logging manual de comidas (hoy no hay forma de escribir a `meal_logs` desde la app, solo de leerlos) - ver seccion 8 para el detalle de "captura por codigo de barras / nombre de comida" que Oscar pidio incluir en el roadmap el 2026-07-17.
