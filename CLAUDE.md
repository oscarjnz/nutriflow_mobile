...
**EL APK YA ESTA PUBLICADO (2026-08-05). La `Latest` es https://github.com/oscarjnz/nutriflow_mobile/releases/tag/v0.1.2**
...
1. **HECHO: el APK esta en GitHub Releases.** Oscar pidio que quedara como historial de versiones, no como un solo release que se sobreescribe, asi que hay tres entradas: `v0.1.0` (solo codigo, sin binario), `v0.1.1` (primer APK firmado) y `v0.1.2` (nombre e icono propios), esta ultima marcada `Latest`. **El `versionCode` solo sube, nunca se repite**: 1, 2, 3 respectivamente.
2. **Google Play: decidido el 2026-08-05 como el paso grande SIGUIENTE al backlog**, no ahora. Oscar quiere primero las notificaciones y las mejoras internas (ver backlog abajo) y despues Play. **Es lo unico que quita el aviso de Play Protect** (ver la entrada de bitacora del 2026-08-05 (release): no tiene arreglo por codigo). Lo que implica: `.aab` en vez de `.apk` (`flutter build appbundle --release --dart-define-from-file=env.json`, misma firma y misma configuracion), porque Play exige App Bundle para apps nuevas desde agosto de 2021; 25 USD una vez; y para cuentas personales creadas despues de 2023-11-13, que es la de Oscar, un test cerrado de 12 testers durante 14 dias antes de poder publicar. Conviene aceptar Play App Signing, que deja el `.jks` como simple upload key y permite resetearla si se pierde.
...
**Tres releases publicados**, porque Oscar pidio explicitamente un historial de versiones y no un solo release que se sobreescribe: `v0.1.0` (la publicacion de solo codigo del 2026-08-01, que tenia tag pero nunca tuvo entrada de release), `v0.1.1` (primer APK firmado) y `v0.1.2` (nombre e icono propios), esta ultima marcada `Latest`. Los dos APK van firmados con la llave real (`CN=Oscar Jimenez`, SHA-256 `d05e43d8…4ab6`, v2 RSA 2048, verificado con `apksigner`, no supuesto). Antes se pushearon los 8 commits que llevaban sin subir.
